...
export default class BurgersRouter {
  constructor(router: express.Router, db: IMonkManager) {
    ...
    router.post("/burger", upload.single('img'), async (req: express.Request, res: express.Response): Promise<void> => {  
      const item = req.body;
      if (!item) {
        res.json({error: true, message: 'Please make sure the burger has all required data...'});
      }
      this.add(collection, db, res, item);
    });
    router.put("/burger", upload.single('img'), async (req: express.Request, res: express.Response): Promise<void> => {
      console.log('in put..........');
      const item = req.body;
      // if (item.length > 1) {
      //   item.map(it => {
    ...
    router.delete("/burgers/", (req: express.Request, res: express.Response) => {
  ...
  async add(collection: ICollection, db: IMonkManager, res: express.Response, doc: Model) { 
  ...
  async update(collection: ICollection, db: IMonkManager, res: express.Response, doc: Model) {
    try {
  ...
  async drop(collection: ICollection, db: IMonkManager, res: express.Response, doc = {}) { 
    try {
